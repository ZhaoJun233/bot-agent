...
把早期桌面版（WinUI 3，**已停止维护，项目只提供 Web 面板**）的 Agent 逻辑抽成**无界面、跨平台、可容器部署**的常驻服务：
...
**“什么时候该搜”是提示词里写死的两份清单**（号主要的是“模型自主判断”，但自主判断需要一个依据）：
...
| **新增** 引用目标的硬规则 | 桌面版只按“触发消息 / 模型指认”取引用目标；headless 版另外：replyTo 的语义写死在提示词（跟谁说话、不是素材）、本轮触发是**复读/模仿**时模型指到别处就不引用、没有触发消息也不引用 |
...
全量约 380 条断言（harness 里 `QQCHAT_IT_ONLY=s19` 可以只跑某个场景）。
