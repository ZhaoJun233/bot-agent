...
public sealed class AgentSessionStore : IAgentSessionStore
{
        ...
        string? piSessionId = null, bool piOwned = true)
    {
        ...
        var session = new AgentSession
        {
            ...
            // 会话的时间戳由**这里**显式写：Domain 的 AgentSession 默认是 default（Domain 不许读时钟）。
            CreatedAt = Clock.Now,
            UpdatedAt = Clock.Now,
        };
    ...
    private static AgentSession? ReadSession(JsonElement item)
    {
        ...
        if (item.TryGetProperty("execs", out var execs) && execs.ValueKind == JsonValueKind.Array)
        {
            ...
            foreach (var exec in execs.EnumerateArray())
            {
                ...
                flat.Add(new AgentSession
                {
                    ...
                    CreatedAt = Time(exec, "createdAt") ?? Clock.Now,
                    UpdatedAt = Time(exec, "updatedAt") ?? Clock.Now,
        ...
        return new AgentSession
        {
            ...
            CreatedAt = Time(item, "createdAt") ?? Clock.Now,
            UpdatedAt = Time(item, "updatedAt") ?? Clock.Now,
