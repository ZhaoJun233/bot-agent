...
public static class AgentMask
{
    ...
    {
        var key = Domain.Qq.Channels.Strip(sourceKey);
        var (isGroup, parsedId) = Domain.Qq.Channels.Parse(sourceKey);
        var id = parsedId > 0
        ...
        var tag = Domain.Qq.Channels.IsOfficial(Domain.Qq.Channels.ChannelOf(sourceKey)) ? "官方" : string.Empty;
