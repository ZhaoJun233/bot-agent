...
public sealed class BotConversation
{
    ...
    /// <summary>
    /// 这条会话属于哪条通道（<see cref="BotAgent.Domain.Qq.Channels.Private"/> / <c>Official</c>）。
    /// 由 SourceKey 的前缀推出 —— 前缀才是隔离的根（老库不带前缀 = 私域，原样可用）。
    /// 快照（<c>ConversationRecord.Channel</c>）里也会写一份，便于外部看库/迁移时不必猜。
    /// </summary>
    public string Channel => BotAgent.Domain.Qq.Channels.ChannelOf(SourceKey);
    ...
    public DateTimeOffset LastTime { get; private set; } = Clock.Now;
        ...
        Action<string, IReadOnlyList<ChatMessage>>? onEvicted = null)
    {
        var kind = Enum.TryParse<ConversationKind>(record.Kind, true, out var k)
            ? k
            : (BotAgent.Domain.Qq.Channels.Parse(record.SourceKey).IsGroup
    ...
    /// <summary>解析 SourceKey → (是否群聊, QQ号/群号)（通道前缀在 <see cref="BotAgent.Domain.Qq.Channels"/> 里吃掉）。</summary>
    public (bool IsGroup, long Id) Target => BotAgent.Domain.Qq.Channels.Parse(SourceKey);
