...
/// </summary>
public sealed class OneBotGateway : IQqChatSource, IQqActions, IDisposable
{
    /// <summary>
    /// 这条路是私域通道（自建协议端 NapCat / OneBot）。
    /// 官方那条是 <c>OfficialBotGateway</c>，两者由 <see cref="ChannelRouter"/> 聚合后交给上层。
    /// </summary>
    ...
    private async Task<JsonNode?> SendActionAsync(string action, string paramsJson, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var done = await Task.WhenAny(tcs.Task, Clock.Delay(TimeSpan.FromSeconds(10), ct));
            if (done != tcs.Task)
            {
                lock (_gate)
                {
    ...
    private void HandleNoticeEvent(JsonNode root)
    {
        ...
        var time = root["time"]?.GetValue<long>() ?? Clock.Now.ToUnixTimeSeconds();
        ...
        if (segments is not null && segments.Count > 0)
        {
            foreach (var seg in segments)
            {
                ...
                switch (type)
                {
                    ...
                    case "reply":
                        // 引用回复：这里不拼正文（“在回哪条”由 BotAgentHost 查上下文后标成 [回复 X「…」]）——
