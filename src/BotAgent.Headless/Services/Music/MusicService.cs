...
public sealed partial class MusicService
{
    ...
    private readonly IMusicRepository _store;
    ...
    public MusicService(IMusicRepository store, NeteaseMusicClient netease, MusicAudioResolver audio, OpenAiClient brain,
        Func<AppSettings> settings, string audioDir, IAudioCache audioCache, Action<string> log)
    {
        _store = store;
        _netease = netease;
        _audio = audio;
    ...
    private void KeepAudio(string key, MusicAudio audio)
    {
        try
        {
            var path = _audioCache.Save(_audioDir, key, audio.Data);
