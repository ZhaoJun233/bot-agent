...
(() => {
  ...
  const TOKEN_KEY = "botagent.panel.token";
  const THEME_KEY = "botagent.theme";

  /* 兼容：面板改名**之前**的旧键名（就是下面 LEGACY_KEYS 里那两串）——
     只读一次用于**迁移**（读到就搬到新键，不回写旧键），免得已经记住的面板令牌丢一次。
     等确认不会再回滚老镜像之后，这段可以删。 */
  const LEGACY_KEYS = { [TOKEN_KEY]: "qqchat.panel.token", [THEME_KEY]: "qqchat.theme" };
  ...
  function storeGet(key) {
    try {
      ...
      if (old) { localStorage.setItem(key, old); return old; }
    } catch { /* 无 localStorage / 隐私模式：当作没有 */ }
    return "";
  }
...
function renderConversations(force) {
  ...
  function bindUi() {
    ...
    // 主题
    const saved = storeGet(THEME_KEY) || "system";
    $("setTheme").value = saved;
    applyTheme(saved);
    $("setTheme").addEventListener("change", (e) => {
      storeSet(THEME_KEY, e.target.value);
  ...
  matchMedia("(prefers-color-scheme: dark)").addEventListener("change", () => {
    if ((storeGet(THEME_KEY) || "system") === "system") applyTheme("system");
  });
