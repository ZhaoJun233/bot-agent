...
- **双通道收单**（2026-09-21）：除了自建 NapCat（私域），还可以同时接 **QQ 官方开放平台**（官方商用，`QQCHAT_OFFICIAL=1` + appid/secret）。两套场景的**会话、上下文、人设、白名单完全隔离**（会话 key 带 `official:` 前缀，官方平台的 openid 会映射成 8e15 起的别名号）；面板里用「全部 / 私域 / 官方商用」三个 Tab 分开看，互不串台
...
另有面板静态与运行时探针（`tests/QQChatAgent.FrontendProbe`）。全量 **473** 条断言；单个场景可以用 `QQCHAT_IT_ONLY=s32` 只跑它。
