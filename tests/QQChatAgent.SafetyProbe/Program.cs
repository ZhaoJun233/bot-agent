...
using QQChatAgent.Services.Permissions;
using QQChatAgent.Services.Rendering;

namespace QQChatAgent.SafetyProbe;
...
///     也用**反射**穿过真实解析器（<c>OpenAiClient.ParseModelOutput</c>）——
///     防止“规则写对了但没接进解析链路”这种假绿。
///
/// 用法：dotnet run --project tests/QQChatAgent.SafetyProbe -c Release
/// </summary>
public static class Program
{
    ...
    {
        Section("P2 · 真实解析器接线（反射穿透 OpenAiClient.ParseModelOutput）");

        var method = typeof(OpenAiClient).GetMethod(
            "ParseModelOutput", BindingFlags.NonPublic | BindingFlags.Static);
        if (method is null)
        {
            Check("找到 ParseModelOutput（私有的模型输出解析入口）", false, "method not found");
            return;
        }

        Check("找到 ParseModelOutput（私有的模型输出解析入口）", true, "ok");

        var silent = Parse(method, "{\"action\":\"silent\",\"reply\":\"我不想说\",\"reasonCode\":\"not_addressed\"}");
    ...
    ///   ③ **接线在**：`BotAgent` 里真的有台账字段与那两个方法（纯逻辑绿 ≠ 接上了链路）。
    /// </summary>
    private static void ApprovalChainTests()
    {
        ...
        var storeField = botType
            .GetFields(BindingFlags.Instance | BindingFlags.NonPublic)
            .FirstOrDefault(f => f.FieldType == typeof(ApprovalStore));
        Check("BotAgent 确实持有审批台账（接线在，不只是纯逻辑）",
            storeField is not null, storeField?.Name ?? "(找不到 ApprovalStore 字段)");

        var handleMethod = botType
            .GetMethods(BindingFlags.Instance | BindingFlags.NonPublic)
            .FirstOrDefault(m => m.Name == "TryHandleApprovalCommand");
        Check("BotAgent 里有“处理审批命令”的方法（且参数是入站消息）",
            handleMethod is not null
            && handleMethod.GetParameters().Any(p => p.ParameterType.Name == "QqChatMessage"),
            handleMethod?.Name ?? "(找不到 TryHandleApprovalCommand)");

        var triggerMethod = botType
            .GetMethods(BindingFlags.Instance | BindingFlags.NonPublic)
            .FirstOrDefault(m => m.Name == "TryOpenApprovalForTool");
        Check("BotAgent 里有“模型想调工具 → 开待批单”的触发点方法",
            triggerMethod is not null, triggerMethod?.Name ?? "(找不到 TryOpenApprovalForTool)");
    ...
    private static void GateAndQuestionTests()
    {
        ...
        // 自己找解析入口（与 ParserWiringTests 同一口径：反射穿透真实链路，防“规则写对但没接线”）
        var parserMethod = typeof(OpenAiClient).GetMethod(
            "ParseModelOutput", BindingFlags.NonPublic | BindingFlags.Static);
        if (parserMethod is null)
        {
            Check("找到 ParseModelOutput（提问那条要过真实解析链路）", false, "method not found");
            return;
        }
        ...
        Check("★ 闸门开关的默认值是**关**（AppSettings 默认 = 与改造前逐字一致）",
            !new QQChatAgent.Services.AppSettings().EnableParticipationGating, "默认 true 就是改线上行为");
        Check("★ 提问开关的默认值是**关**",
            !new QQChatAgent.Services.AppSettings().EnableQuestions, "默认 true 就是改线上行为");
        ...
        var parsedAskDefault = Parse(parserMethod, """{"action":"ask","reply":"要不要我去查一下？"}""");
    ...
    private static CompletionResult Parse(MethodInfo method, string raw)
    {
        // 第二个参数是「允许提问」开关：默认关（与线上默认一致 —— action=ask 仍是安全静默）
        var value = method.Invoke(null, new object?[] { raw, false });
        if (value is null)
        {
            throw new InvalidOperationException("ParseModelOutput 返回 null");
        }
