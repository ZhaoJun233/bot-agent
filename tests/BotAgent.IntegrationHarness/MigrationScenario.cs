...
public static partial class Program
{
    private static async Task RunMigrationScenarioAsync()
    {
        ...
        // 自己发过的消息台账（老版本是 data/own-messages.json）：批次 3 起改成 own_messages 表。
        // 两条旧记录 + 一条"JSON 里没有 at"的（老格式可能缺，导入时按最早处理，不该整条丢掉）。
        await File.WriteAllTextAsync(Path.Combine(dataSub, "own-messages.json"),
            $$"""
            [
              { "id": 7701, "text": "迁移前我说过的第一句", "at": "{{now.AddDays(-2):O}}" },
              { "id": 7702, "text": "迁移前我说过的第二句", "at": "{{now.AddDays(-1):O}}" },
              { "id": 7703, "text": "这条老数据没写时间" }
            ]
            """, Encoding.UTF8);
        ...
        // ---- 自己发过的消息：老 JSON 进库 + 旧文件留档 + 新发的也记账 ----
