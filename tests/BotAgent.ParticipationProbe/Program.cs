...
namespace BotAgent.ParticipationProbe;
...
///
/// 用法：dotnet run --project tests/BotAgent.ParticipationProbe -c Release
/// </summary>
public static class Program
{
    ...
    public static int Main()
    {
        ...
        // ── 17) 策略热更新 + 入站映射 + 接线（面板改设置那条路 / BotAgentHost 真的喂了事件） ──
    ...
    private static void PolicyUpdateTests(DateTimeOffset t0)
    {
        ...
        // ── 16) 反射钉死：用例真的把事件喂进去了（接线在，不只是纯逻辑） ──
        // 2026-09-23 改锚点：这几条原来找 BotAgentHost 上的成员（当年它是面板 façade + 参与宿主）。
        // 批次 5 之后参与整块在 ParticipationUseCase 上，所以目标类跟着实现走；断言意图一字未改。
        var useCaseType = typeof(BotAgent.Services.Participation.ParticipationUseCase);
        var observeMethod = useCaseType
            .GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
            .FirstOrDefault(mm => mm.Name == "Observe");
        Check("参与用例里有喂事件的方法（参数是会话 key + 事件）",
            ...
            observeMethod?.ToString() ?? "(找不到 Observe)");

        // 台账字段是**私有**的（用例只暴露 Count 与 Snapshot 两个口子），所以这条只能靠反射看
        var useCaseFields = useCaseType.GetFields(BindingFlags.Instance | BindingFlags.NonPublic);
        Check("参与用例持有参与台账（接线在）",
            useCaseFields.Any(f => f.FieldType == typeof(ParticipationRegistry)),
            string.Join(",", useCaseFields.Select(f => f.FieldType.Name).Where(n => n.Contains("Participation"))));

        var policyMethod = useCaseType
            .GetMethods(BindingFlags.Instance | BindingFlags.NonPublic)
            .FirstOrDefault(mm => mm.Name == "BuildPolicy");
        Check("参与用例会从设置里拼参与上限（面板那 5 个参数真的被读）",
            policyMethod is not null && policyMethod.ReturnType == typeof(ParticipationPolicy),
            policyMethod?.ToString() ?? "(找不到 BuildPolicy)");
    }
