...
namespace QQChatAgent.PipelineEval;
...
public static class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        QQChatAgent.Services.FileLog.WriteToFile = false;

        Console.WriteLine("QQ Chat Agent · 隔离评测（确定性管线回归）");
        ...
        {
            Console.WriteLine("✗ 找不到 OpenAiClient.ParseModelOutput（解析链路签名变了）→ 失败关闭");
            return 2;
        }
        ...
        foreach (var file in new[] { "on-demand.json", "research.json", "social.json" })
        {
            ...
            foreach (var node in doc["cases"]?.AsArray() ?? new JsonArray())
            {
                ...
                var result = RunCase(parserMethod, category, c);
                cases.Add(result);
    ...
    private static CaseResult RunCase(MethodInfo parserMethod, string category, JsonObject c)
    {
        ...
        try
        {
            ...
            {
                "decision" => RunDecision(parserMethod, category, id, title, mechanism, fault, c, expect),
    ...
    private static CaseResult RunDecision(
        MethodInfo parserMethod, string category, string id, string title, string mechanism, bool fault,
        JsonObject c, JsonObject expect)
    {
        ...
        if (driver == "parser")
        {
            ...
            var parsed = (CompletionResult)(parserMethod.Invoke(
                                                null,
                                                new object?[] { raw, c["questionsEnabled"]?.GetValue<bool>() ?? false })
                                            ?? throw new InvalidOperationException("解析返回 null"));
    ...
    private static string FindRepoRoot()
    {
        ...
        while (dir is not null)
        {
            if (Directory.Exists(Path.Combine(dir.FullName, "tests", "fixtures", "pipeline")) ||
                File.Exists(Path.Combine(dir.FullName, "QQChatAgent.sln")))
            {
                return dir.FullName;
            }
